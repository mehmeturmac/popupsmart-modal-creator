function Pop() {
  var cssRuleFile = 'http://localhost:3000/modals/modal6.css';
  ...
  let createPopUp = function () {
    if (typeof conDivObj === 'undefined') {
      ...
      conDivObj.setAttribute('id', 'modal6');
    }
    ...
    conDivObj.innerHTML = `
    <div style="color: ${colors.color1}; background-color: ${colors.color3};" id="modal6-popup" class="modal6-popup hide ${
      ...
      <div class="btnGroup">
        <button id="submitBtn" style="color: rgba(255, 255, 255, 0.9); background-color: ${colors.color3}; border-style: solid; border-width: 1px; border-color: ${colors.color1}">${
      contents.content3
    }</button>
        <button id="closeBtn2" style="color: ${colors.color2}; background-color: ${colors.color1};">
          ${contents.content4}
        </button>
      </div>
    </div>`;
    ...
    function actions() {
      ...
      const popup = document.getElementById('modal6-popup').classList;
      ...
      if (webHook.url) {
        ...
        closeBtn2.addEventListener('click', () => {
          let formData = contents.content4;
          if (webHook.sendForm) sentData = { ...sentData, formData };
          if (webHook.sendClick) sentData = { ...sentData, sendData };
          webHookPost(sentData);
        });
