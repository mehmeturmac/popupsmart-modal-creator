function Pop() {
  var cssRuleFile = 'http://localhost:3000/modals/modal2.css';
  ...
  let contents = {
    content0: 'PLANS',
    content1: 'Choose best for you',
    content2: 'Only pay for the capacity that you use.',
    content3: 'Starter',
    content4: '1 free (then $15 per meember / month)',
    content5: 'Pro',
    content6: '$19 per member/month',
    content7: 'Business',
    content8: '$29 per member/month',
    content9: 'Cancel',
    content10: 'Continue',
  };
  ...
  let appearance = { size: 'medium', position: '4', afterXSec: 0, afterScroll: 0, trafficSource: '', exitIntent: false };
  ...
  let createPopUp = function () {
    if (typeof conDivObj === 'undefined') {
      ...
      conDivObj.setAttribute('id', 'modal2');
    }
    ...
    conDivObj.innerHTML = `
    <div style="color: ${colors.color2}; background-color: ${colors.color1};" id="modal2-popup" class="modal2-popup hide ${
      appearance.size === 'small' ? 'small' : appearance.size === 'medium' ? 'medium' : 'large'
    }">
      <button class="closeBtn" id="closeBtn" >${closeBtn}</button>
      <h6 style="color: ${colors.color3};">${contents.content0}</h6>
      <h3>${contents.content1}</h3>
      <h5>${contents.content2}</h5>
      <div class="radioGroup">
        <label for="radio1">
          <input type="radio" name="popupGroup" id="radio1" />
          <span>${contents.content3}</span>
        </label>
        <p>${contents.content4}</p>
        <label for="radio2">
          <input type="radio" name="popupGroup" id="radio2" />
          <span>${contents.content5}</span>
        </label>
        <p>${contents.content6}</p>
        <label for="radio3">
          <input type="radio" name="popupGroup" id="radio3" checked />
          <span>${contents.content7}</span>
        </label>
    ...
    function actions() {
      ...
      const popup = document.getElementById('modal2-popup').classList;
      ...
      if (webHook.url) {
        ...
        let formData;
        document.getElementById('radio1').addEventListener('click', () => (formData = contents.content3));
        document.getElementById('radio2').addEventListener('click', () => (formData = contents.content5));
        document.getElementById('radio3').addEventListener('click', () => (formData = contents.content7));
