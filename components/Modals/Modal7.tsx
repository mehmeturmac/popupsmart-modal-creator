import styles from './Modal7.module.scss';
...
export default function Modal7() {
  ...
  return (
    <div style={{ color: colors.color1, backgroundColor: colors.color3 }} className={`${styles.modal7} ${size === 'small' ? styles.small : size === 'medium' ? styles.medium : styles.large}`}>
      <button className={styles.closeBtn}>{closeBtn}</button>
      <h3>{contents.content1}</h3>
      <h5>{contents.content2}</h5>
      <input type="text" placeholder={contents.content3} style={{ color: colors.color1, backgroundColor: colors.color3 }} />
      <label htmlFor="popup-checkbox">
        <input type="checkbox" id="popup-checkbox" />
        <span>{contents.content4}</span>
      </label>
      <div className={styles.btnGroup}>
        <button style={{ color: colors.color2, backgroundColor: colors.color1 }}>{contents.content5}</button>
      </div>
    </div>
  );
}
