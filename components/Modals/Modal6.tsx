import styles from './Modal6.module.scss';
...
export default function Modal6() {
  ...
  return (
    <div style={{ color: colors.color1, backgroundColor: colors.color3 }} className={`${styles.modal6} ${size === 'small' ? styles.small : size === 'medium' ? styles.medium : styles.large}`}>
      <button className={styles.closeBtn}>{closeBtn}</button>
      <h3>{contents.content1}</h3>
      <h5>{contents.content2}</h5>
      <div className={styles.btnGroup}>
        <button style={{ color: 'rgba(255, 255, 255, 0.9)', backgroundColor: colors.color3 }} className={`border`}>
          {contents.content3}
        </button>
        <button style={{ color: colors.color2, backgroundColor: colors.color1 }}>{contents.content4}</button>
      </div>
    </div>
  );
}
