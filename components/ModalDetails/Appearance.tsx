    ...
    <div className={styles.appearance}>
      ...
      <div className={styles.size}>
        ...
        <div className={styles.sizeGroup} role="group">
          {['small', 'medium', 'large'].map((x, i) => (
            <div key={i}>
              <input type="radio" id={`size${i}`} name="sizeGroup" checked={x === size ? true : false} onChange={() => setSize(x)} />
      ...
      <div className={styles.position}>
        ...
        <div className={styles.positionGroup} role="group2">
          {[...Array(9)].map((x, i) => (
            <label key={i}>
              <input type="radio" id={`position${i}`} name="positionGroup" checked={i === position ? true : false} onChange={() => setPosition(i)} />
              <span></span>
            </label>
          ))}
        </div>
